...
const getPrePoQuote = async (fromChain: ChainId, fromToken: string, userAddress: string, amount: string): Promise<Step> => {
  const DEPOSIT_TRADE_HELPER_ETHEREUM_CONTRACT_ARB = '0x2fa567576f9250666E3D81984Ad3b373028a881C'
  const DEPOSIT_TRADE_HELPER_ABI = [{ "inputs": [{ "internalType": "address", "name": "recipient", "type": "address" }, { "components": [{ "internalType": "uint256", "name": "amountOutMinimum", "type": "uint256" }, { "internalType": "uint256", "name": "deadline", "type": "uint256" }], "internalType": "struct IDepositTradeHelper.OffChainBalancerParams", "name": "balancerParams", "type": "tuple" }], "name": "wrapAndDeposit", "outputs": [], "stateMutability": "payable", "type": "function" }]
  const DEPOSIT_TRADE_HELPER_GAS_LIMIT = '2000000'

  const amountOutMinimum = ethers.BigNumber.from(amount).mul(85).div(100).toString()
  const deadline = Date.now() + 3600

  // contract call
  const contract = new ethers.Contract(
    DEPOSIT_TRADE_HELPER_ETHEREUM_CONTRACT_ARB, DEPOSIT_TRADE_HELPER_ABI
  )
  const stakeTx = await contract.populateTransaction.wrapAndDeposit(userAddress, [amountOutMinimum, deadline])
