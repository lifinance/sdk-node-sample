...
const getMultihopQuote = async (fromChain: ChainId, fromToken: string, toChain: ChainId, toToken: string, usdAmount: number, userAddress: string): Promise<Step> => {
  ...
  const quoteRequest: ContractCallQuoteRequest = {
    ...
    toContractGasLimit: secondBridgeQuote.transactionRequest!.gasLimit!.toString(),
  }
...
const run = async () => {
  ...
  try {
    ...
    // get quote
    const quote = await getMultihopQuote(fromChain, fromToken, toChain, toToken, usdAmount, await signer.getAddress())
