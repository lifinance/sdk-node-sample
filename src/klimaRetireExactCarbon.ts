...
const getKlimaQuote = async (fromChain: ChainId, fromToken: string, userAddress: string, retireAmount: string): Promise<Step> => {
  ...
  const contract = new ethers.Contract(
    KLIMA_ETHEREUM_CONTRACT_OPT, KLIMA_ABI, provider
  )
...
const run = async () => {
  ...
  try {
    ...
    // get quote
    const quote = await getKlimaQuote(fromChain, fromToken, await signer.getAddress(), retireAmount)
